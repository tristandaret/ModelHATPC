...
def ratio(RC, z, d, phi, L):
    ...
    # print(f"Left:   ({xmin:.2f},{y_xmin:.2f})")
    # print(f"Right:  ({xmax:.2f},{y_xmax:.2f})")
    # print(f"Bottom: ({x_ymin:.2f},{ymin:.2f})")
    # print(f"Top:    ({x_ymax:.2f},{ymax:.2f})")

    L = np.sqrt((y[1] - y[0]) ** 2 + (x[1] - x[0]) ** 2)

    for i in range(len(x)):
        print(f"({x[i]-xwidth/2:.3f},{y[i]-ywidth/2:.3f})")
